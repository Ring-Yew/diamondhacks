...
async def get_prohibited_items(body: ProhibitedItemsRequest):
    ...
    Uses DeepSeek to pull from current knowledge of regulations.
    """
    client = get_anthropic_client()

    system_prompt = "You are a travel compliance expert. Return only valid JSON with no prose."

    prompt = f"""List prohibited items for each destination and transport mode.
